...
<%
    String apiKey = "7961459716"; // 발급받은 API 키 사용
    int maxPage = 1; // 총 80건이므로 1페이지
    JSONParser parser = new JSONParser();
    JSONArray allData = new JSONArray();

    String jsonFilePath = application.getRealPath("/data/restVentureData.json");
    ...
    try {
        for (int pageNo = 1; pageNo <= maxPage; pageNo++) {
            String apiUrl = "https://data.ex.co.kr/openapi/restinfo/restVentureList"
                          + "?key=" + apiKey
                          + "&type=json"
                          + "&numOfRows=99"
                          + "&pageNo=" + pageNo;
        ...
        out.println("<h3>청년매장현황 데이터 저장 성공!</h3>");
        out.println("<p>총 청년창업매장 수: " + allData.size() + "</p>");
