...
<%
    ...
    infoDto dto=infodao.getData(id);
...
<div class="form-wrapper">
  ...
  <form id="eventForm" action="updateaction.jsp" method="post" enctype="multipart/form-data">
    <!-- 이벤트명 -->
    <div class="mb-4">
    ...
    <input type="hidden" name="readcount" value="<%=dto.getReadcount() %>">
      ...
      <input type="text" id="title" name="title" placeholder="제목을 입력하세요" required  value="<%=dto.getTitle()%>"/>
    </div>
    ...
    <div class="mb-4">
      ...
      <select name="hgId" id="hgId" required>
        ...
        <% for (Map<String, String> rest : restList) { 
        	 String restId = rest.get("id");
             String selected = restId.equals(dto.getHgId()) ? "selected" : "";
        %>
          <option value="<%= restId %>" <%= selected %>><%= rest.get("rest_name") %></option>
        <% } %>
      </select>
    </div>
    ...
    <div class="mb-4">
      ...
      <textarea id="content" name="content" placeholder="내용을 입력하세요" required ><%=dto.getContent()%></textarea>
    </div>

    <!-- 버튼 (세로 정렬) -->
    <div class="text-center" style="float:right">
      <button type="submit" id="submitBtn" class="btn btn-success">수정</button>      
